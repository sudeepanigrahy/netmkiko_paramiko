...
def hosttohost(dtpo1, dtpo3, in_acl_comm, out_acl_comm, ssubnet_IPs, dsubnet_IPs, subnet_s):
    ...
    if in_acl_comm=='not_present' and type(dtpo1)==type(pd.DataFrame([])):
        ...
        bool19 = dtpo1[6] != 'any'
        bool20 = dtpo1[6] != 'host'
        allowtohost_dtpo2 = dtpo1[bool13 & bool14 & bool15 & bool16 & bool17 & bool19 & bool20]
        ...
        for s_ip in ssubnet_IPs:
            for d_ip in dsubnet_IPs:
                ...
                length_of_allowedtohost_dtpos = int(len(allowedtohost_dtpo)+len(allowedtohost_dtpo1)+len(allowedtohost_dtpo2)+len(allowedtohost_dtpo3))
                if count > 0 or length_of_allowedtohost_dtpos==0:
                    print(f"permit ip host {s_ip} host {d_ip}")
                    acl_entries_in.append(f"permit ip host {s_ip} host {d_ip}")
                    
        #print(acl_entries_in)
        print('\n')
    ...
    if out_acl_comm=='not_present' and type(dtpo3)==type(pd.DataFrame([])):
        ...
        allowtohost_dtpo2 = dtpo1[bool13 & bool14 & bool15 & bool16 & bool17 & bool18 & bool19]
        allowtohost_dtpo2 = allowtohost_dtpo2.reset_index(drop=True)
            ...
            if len(allowed_dtpo3) > 0:
                for row in range(len(allowed_dtpo3)):
                    if allowed_dtpo3.loc[row][1]=="permit" and allowed_dtpo3.loc[row][2]=="ip":
                        #print(f"There is already existing in-comm between these two subnets: {subnet_s} and {subnet_d}")
                        print("Even though there is a 'permit ip any any' entry, creating entries for L3's tracking purposes")
                        comm_presence['in_acl_comm'] = 'not_present'
                    else: pass                
            ...
            if len(allowed_dtpo3) > 0:
                for row in range(len(allowed_dtpo3)):
                    if allowed_dtpo3.loc[row][1]=="permit" and allowed_dtpo3.loc[row][2]=="ip":
                        #print(f"There is already existing out-comm between these two subnets: {subnet_s} and {subnet_d}")
                        print("Even though there is a 'permit ip any any' entry, creating entries for L3's tracking purposes")
                        comm_presence['out_acl_comm'] = 'not_present'
                    else: pass   
...
def ipaclchecker(sources_grouped_details, destinations_grouped_details, direction):
    ...
    for group_s, detail_s in sources_grouped_details.items():
        #if detail_s['group_L3'] == '10.195.1.74': continue
        ...
        subnets_entries[subnet_s_str] = {'in_acls' : [], 'in_numbering' : [], 'in_acl_name' : '', 'deny_in' : '',  'out_acls' : [], \
                                         'out_numbering' : [], 'out_acl_name' : '', 'deny_out' : '', 'subnet_primary' : primary_l3}
        ...
        if detail_s['group_acls']['in']!='not_present':
            ...
            lastentryin = list(dtpo1.tail(1).values[0])
            if 'deny' in lastentryin and 'ip' in lastentryin and 'any' in lastentryin:
                subnets_entries[subnet_s_str]['deny_in'] = 'yes'
            else: subnets_entries[subnet_s_str]['deny_in'] = 'no'
        ...
        if detail_s['group_acls']['out']!='not_present':
            ...
            lastentryout = list(dtpo1.tail(1).values[0])
            if 'deny' in lastentryout and 'ip' in lastentryout and 'any' in lastentryout:
                subnets_entries[subnet_s_str]['deny_out'] = 'yes'
            else: subnets_entries[subnet_s_str]['deny_out'] = 'no'
