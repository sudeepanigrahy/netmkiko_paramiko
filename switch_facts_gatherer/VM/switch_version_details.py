...
with open("outputdatafile.csv", "w", newline='') as filetypeobject:
...
def ios(connection, switch, version, up):  
    ...
    with open("outputdatafile.csv", "a", newline='') as filetypeobject:
        header = ["switchname", "version", "serial", "model", "uptime"]
        dictwritertypeobject = csv.DictWriter(filetypeobject, fieldnames = header)
...
def reachability(switchlist):        
    ...
    for switch in switchlist:
        try:
            kwargs = {
                'device_type': 'cisco_ios','ip': switch,'username': 'spanigrahy','password': 'Sueme@0124','port':'22'
                }
            ...
            try:
                kwargs = {
                    'device_type': 'cisco_ios','ip': switch,'username': 'spanigrahy','password': 'Sueme@0124','port':'23'
                    }
                connection = ConnectHandler(**kwargs)
                differentiator(connection, switch)
            except:
...
def threading():
    ...
    for file in range(1,11):                
        with open(f"{file}.csv", "r") as filetypeobject:
            header = ["switchname"]
            ...
            thread = threading.Thread(target=reachability, args=(inputswitchlist,))
            thread.start()
...
schedule.every().tuesday.at("05:10").do(threading)
while True:
