...
global h
h = ['AS Hostname', 'Spanning-tree Root', 'Root_Port', 'Root_Port==>', 'Secondary_Root_Port', 'Secondary_Root_Port==>', 'comments']
...
def work(connection, switch, t):    
    ...
    four = incoming_data[3]
    five = incoming_data[4]
    
    with open(f"output{t}.csv", "a", newline='') as filetypeobject:
        header = h
        dictwritertypeobject = csv.DictWriter(filetypeobject, fieldnames = header)
        dictwritertypeobject.writerow({h[0]: switch , h[1]: zero, h[2]: one, h[3]: four, h[4]: two, h[5]: five, h[6]: three})
...
def prework(csv_chunk, t):
    ...
    for i in inputswitchlist:
        ...
        try:
            kwargs = {
                'device_type': 'cisco_ios','ip': i,'username': 'spanigrahy','password': 'Sueme@1123','port':'22'
                }
            connection = ConnectHandler(**kwargs)
        except:
            try:
                kwargs = {
                    'device_type': 'cisco_ios_telnet','ip': i,'username': 'spanigrahy','password': 'Sueme@1123','port':'23'
                    }
                connection = ConnectHandler(**kwargs)
...
final_DF.to_csv("STP_ROOT_AUDIT.csv")
