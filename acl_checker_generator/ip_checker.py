...
def ipchecker():
    ...
    print('Lets check the reachability of the IPs:\n')
    ...
    if unreachables_sources: 
        print(f"\nHi {user.capitalize()}, these IP/s are unreachable from the source side, please check with the user, \
    or remove from the files and run code again:\n", unreachables_sources, '\n')
        exit()
    
    if unreachables_destinations:
        print(f"\nHi {user.capitalize()}, these IP/s are unreachable from the destination side, please check with the user, \
    or remove from the files and run code again:\n", unreachables_destinations, '\n')
        exit()
        
    else: print(f"\nHi {user.capitalize()}, these IPs are all okay, and can be passed to the next section:\n"+"\nSource_IPs:\n",\
                reachables_sources,"\n"+"Destination_IPs:\n",reachables_destinations, sep="")
    
    return (reachables_sources, reachables_destinations)
